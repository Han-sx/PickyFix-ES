...
_INLINE_ ret_t
reencrypt(OUT pad_ct_t ce, OUT split_e_t *e2, IN const split_e_t *e, IN const ct_t *l_ct) {
...
crypto_kem_keypair(OUT unsigned char *pk, OUT unsigned char *sk) {
    ...
    // Sample the sigmas
    GUARD(
        sample_uniform_r_bits_with_fixed_prf_context(&l_sk->sigma0, &s_prf_state, NO_RESTRICTION));
    GUARD(
        sample_uniform_r_bits_with_fixed_prf_context(&l_sk->sigma1, &s_prf_state, NO_RESTRICTION));
...
crypto_kem_dec_bgf(OUT unsigned char *ss, IN const unsigned char *ct, IN const unsigned char *sk) {
    ...
    DMSG("  Decoding.\n");
    uint32_t dec_ret = decode_bgf(&e, &syndrome, l_ct, l_sk) != SUCCESS ? 0 : 1;
